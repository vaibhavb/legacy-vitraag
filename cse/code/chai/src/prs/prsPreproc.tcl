...
proc prs_preproc {filename} {
    ...
    if {[catch {set fw [open $CHAITMPDIR/$f.p "w"]}]} {
	error "Cannot write $CHAITMPDIR/$f.p"
    }

    # set the mode so that it can be writable by other people. 
    catch {exec chmod a+w $CHAITMPDIR/$f.p}
...
docproc read_spec {filename} {
	...
	set tmpFilename $CHAITMPDIR/mocha$i.spec
	...
	append cmd $i _read " " $CHAITMPDIR/mocha$i.spec
    ...
    # foreach i [array names fd] {
    #	catch {unset cmd}
    #	append cmd "exec rm " $CHAITMPDIR/mocha$i.spec
    #	catch {eval $cmd}
    #}
