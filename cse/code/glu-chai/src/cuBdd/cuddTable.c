...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddTable.c,v 1.1.1.1 2003/04/24 23:35:11 ashwini Exp $";
#endif
...
#pragma pointer_size restore
	...
	if ((unsigned) deleted != unique->constants.dead) {
	    ddReportRefMess(unique, CUDD_MAXINDEX, "cuddGarbageCollect");
	}
	totalDeleted += deleted;
  ...
  CUDD_VALUE_TYPE  value)
{
	...
	(10 * unique->constants.dead > 9 * unique->constants.keys))) { 	/* too many dead */
	    ...
	    cuddRehash(unique,CUDD_MAXINDEX);
	}
    }
  ...
  int i)
{
	...
	if (cuddGarbageCollect(unique,1)) return;
    }
