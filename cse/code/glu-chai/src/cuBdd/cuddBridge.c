...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddBridge.c,v 1.1.1.1 2003/04/24 23:35:11 ashwini Exp $";
#endif
...
static DdNode * addBddDoPattern ARGS((DdManager *dd, DdNode *f));
  ...
  DdNode * f)
{
    ...
    T = addBddDoPattern(dd,fv);
    if (T == NULL) return(NULL);
    cuddRef(T);

    E = addBddDoPattern(dd,fvn);
    if (E == NULL) {
	Cudd_RecursiveDeref(dd, T);
	return(NULL);
    }
