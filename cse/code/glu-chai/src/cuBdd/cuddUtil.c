/**CFile***********************************************************************
  ...
  Description [External procedures included in this module:
		...
		<li> Cudd_bddPickOneCubeRandom()
		<li> Cudd_bddPickOneMintermRandom()
...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddUtil.c,v 1.3 2003/04/25 01:27:41 ashwini Exp $";
#endif
...
/**Function********************************************************************
  ...
  Synopsis    [Picks one on-set cube truly randomly from the given DD.]
  ...
  SeeAlso     [Cudd_bddPickOneMintermRandom]

******************************************************************************/
int
Cudd_bddPickOneCubeRandom(
  DdManager * ddm,
  DdNode * node,
  char * string)
{
	...
	if( dir != 0) {
	    /* Choose the T branch */
	    if( T == one) {
		/* found a valid cube in the randomly chosen
		 * direction.*/
	        string[N->index] = 1;
		break;
	    } 
	    if ( T == bzero) {
		/* found a zero in the randomly chosen
		 * direction. Take the other branch.*/
		string[N->index] = 0;
		if( E == one) {
		    /* T branch is a bzero and E
		     * a one. Chose E and exit. */
		    break;
		}
		node = E;
	    } else {
		...
		string[N->index] = 1;
		if( T == one) {
		    /* E branch is a bzero and T
		     * a one. Chose T and exit. */
		    break;
		}
		node = T;
	    } else {
...
} /* end of Cudd_bddPickOneCubeRandom */
...
/**Function********************************************************************
  ...
  SeeAlso     [Cudd_bddPickOneCubeRandom]

******************************************************************************/
DdNode *
Cudd_bddPickOneMintermRandom(
  DdManager * dd /* manager */,
  DdNode * f /* function from which to pick one minterm */,
  DdNode ** vars /* array of variables */,
  int  n /* size of <code>vars</code> */)
{
    ...
    result = Cudd_bddPickOneCubeRandom(dd,f,string);
    if (result == 0) {
	FREE(string);
	FREE(indices);
	return(NULL);
    ...
    /* Randomize choice for don't cares. But NO - this is a bad idea for
     *        simulation.
     *        
    for (i = 0; i < n; i++) {
	if (string[indices[i]] == 2) 
...
}  /* end of Cudd_bddPickOneMintermRandom */
