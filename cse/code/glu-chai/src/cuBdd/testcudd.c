...
#define TESTCUDD_VERSION	"TestCudd Version #0.9, Release date 9/14/98"
...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: testcudd.c,v 1.1.1.1 2003/04/24 23:35:11 ashwini Exp $";
#endif
...
main(int argc, char **argv)
{
	...
	if (pr>2) {
		...
		if (Cudd_IsConstant(node)) {
			...
			(unsigned long) node / (unsigned long) sizeof(DdNode),
			Cudd_V(node));
#else
		    (void) printf("ID = 0x%x\tvalue = %-9g\n",
			(unsigned int) node / (unsigned int) sizeof(DdNode),
			Cudd_V(node));
#endif
		} else {
#if SIZEOF_VOID_P == 8
...
/**Function********************************************************************
  ...
  Synopsis    [Tests the functions realted to the exclusive OR.]

  Description [Tests the functions realted to the exclusive OR. It
