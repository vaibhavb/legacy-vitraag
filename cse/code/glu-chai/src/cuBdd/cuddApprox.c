...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddApprox.c,v 1.1.1.1 2003/04/24 23:35:11 ashwini Exp $";
#endif
...
/**Function********************************************************************
  ...
  is a function. This procedure tries to approximate wherer the bias is 0
  ...
  DdHashTable *cache)
{
    ...
    if (f->ref != 1 && (res = cuddHashTableLookup2(cache,f,b)) != NULL) {
	if (res->ref == 0) {
	    cache->manager->dead++;
    ...
    if (!BAapplyBias(dd, Ft, Bt, info, cache))
	return(0);
