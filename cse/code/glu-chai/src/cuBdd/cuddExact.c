...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddExact.c,v 1.1.1.1 2003/04/24 23:35:11 ashwini Exp $";
#endif
  ...
  int  upper)
{
    ...
    int  subsetCost;
    int  size, unused, nvars, level, result;
    ...
    */
...
#endif
	newSubsets = 0;
	level = size - k;		  /* first bottom variable */
	...
	for (i = 0; i < oldSubsets; i++) { /* for each subset of size k-1 */
		...
		if (table->subtables[j-1].keys == 1 &&
		    table->vars[table->invperm[j-1]]->ref == 1) continue;
...
/**Function********************************************************************
  ...
  of subsequent reorderings. THe format used is the same of the next fields:
...
/**Function********************************************************************
  ...
  Description [Check whether a variable is the one with the highest index
