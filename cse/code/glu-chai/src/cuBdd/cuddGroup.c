...
#ifndef lint
static char rcsid[] DD_UNUSED = "$Id: cuddGroup.c,v 1.1.1.1 2003/04/24 23:35:11 ashwini Exp $";
#endif
...
static int ddGroupSifting ARGS((DdManager *table, int lower, int upper, int (*checkFunction)(DdManager *, int, int)));
static void ddCreateGroup ARGS((DdManager *table, int x, int y));
static int ddGroupSiftingAux ARGS((DdManager *table, int x, int xLow, int xHigh, int (*checkFunction)(DdManager *, int, int)));
...
static int ddGroupSiftingBackward ARGS((DdManager *table, Move *moves, int size));
  ...
  int (*checkFunction)(DdManager *, int, int))
{
    ...
    qsort((void *)var,classes,sizeof(int),(int (*)(const void *, const void *))ddUniqueCompareGroup);
	...
	merged = 0;
	if (table->groupcheck == CUDD_GROUP_CHECK7) {
  ...
  Move ** moves)
{
    ...
    ** What we use here is not a real lower bound, because we ignore
